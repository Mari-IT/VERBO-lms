...
export const getUserEnrolledCourses = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch enrolled courses.", error });
  }
};
...
export const getUserCourseProgress = async (
  ...
  try {
    ...
    if (!progress) {
      res
        .status(404)
        .json({ message: "Course progress data is missing for this user." });
      return;
    }
    res.json({
      message: "Course progress loaded successfully.",
      data: progress,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Failed to fetch user course progress.", error });
  }
};
